...
def normalizeString(s):
    s = s.lower()
    s = re.sub(r"([.!?])", r" \1", s) # force a space between punctuations
    s = re.sub(r"[^a-zA-Z.!?]+", r" ", s) # erase all non-letter characters and except for  . ! ?
    return s
...
#    print(list(input_batch.size()))
#    print(input_batch)

    print ("lengths : ")
#    print(lengths.type())
#    print(list(lengths.size()))
#    print(lengths)
...
#store word2idx data into a csv file
def storeVocData(voc):
    #print(voc.word2index)
    #print(len(voc.word2index))
    #print(len(voc.index2word))
    with open("word2idx.txt", 'w') as fp:
        for key in voc.word2index:
            fp.write(key+","+str(voc.word2index[key])+"\n")

    with open("idx2word.txt", 'w') as fp:
        for key in voc.index2word:
            fp.write(str(key)+","+voc.index2word[key]+"\n")

def initDic(checkpoint_to_load):
    print("initDic called with path: "+checkpoint_to_load)
    corpus_name = "cornell movie-dialogs corpus"
    loadFilename = checkpoint_to_load
    checkpoint = torch.load(loadFilename, map_location=torch.device('cpu'))
    voc = Voc(corpus_name)
    voc.__dict__ = checkpoint['voc_dict']
    return voc

def preprocessData(sentence, voc, MAX_LEN): #TODO : 
    input_sentence = normalizeString(sentence)
...
def postprocessData(output_words):
    output_words[:] = [x for x in output_words if not (x == 'EOS' or x == 'PAD')]
